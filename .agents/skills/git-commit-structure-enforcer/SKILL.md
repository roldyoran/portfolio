---
name: git-commit-structure-enforcer
...
If the commit affects multiple files or benefits from additional context, include a commit body with a structured file summary.
...
Files:
- [ADD] path/to/file1.ts, path/to/file2.ts
- [MOD] path/to/file3.ts, path/to/file4.ts
- [DEL] path/to/file5.ts
- [REN] old/path.ts -> new/path.ts
```
...
```text
feat(api): Add user activity endpoint

Files:
- [ADD] src/routes/activity.ts, src/services/activity.ts
- [MOD] src/controllers/user.ts
```
...
When a commit body exists, include a `Files:` section after the summary.

Rules:

- Include all created, modified, deleted, or renamed files in the commit
- Use relative paths
...
Files:
- [ADD] path/to/file1.ts, path/to/file2.ts
- [MOD] src/auth/login.ts, src/auth/jwt.ts
- [DEL] src/utils/legacy.ts
- [REN] src/ui/button.ts -> src/ui/Button.ts
```

Example:

```text
fix(auth): Refresh token before request retry
